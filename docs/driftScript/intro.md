...
- Comparison operators and dictionaries reworked to use `_eq`, `_cmp`, and `_hash`
    ...
    - Added function `int hash(obj1, obj2, ..)` that hashes and combines objects/primitives
    ...
    - Array and Dictionary `==`/`!=` operators invoke sub-object's `_eq`/`_cmp` meta-functions (recursively).
