...
- Global variables defined in this script are not accessible from the `dw_sim_*` functions
- Should be defined in `/MapInit.DriftScript`

## dw_sim_init
```sq
function dw_sim_init() {
    # your code here
}
```
...
- This function is only called once.
- Global variables defined here are accessible to `dw_sim_update`
- At this point, {{fn("dw_get_sim_tick")}} will return 0.
...
- On first invocation, {{fn("dw_get_sim_tick")}} will return 1.
    - On second invocation, {{fn("dw_get_sim_tick")}} will return 2, and so on..
- This is where your main script code goes.
- Global variables persist across `dw_sim_update` calls.
- Should be defined in `/SimUpdate.DriftScript`
